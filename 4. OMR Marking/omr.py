...
# load the image then convert it to grayscale, blur it and, then find edges
...
# find contours in the edged image, then initialize the contour that corresponds to the document
...
for c in cnts:
    ...
    # if approximated contour has four points,then we can assume we have found the paper
    if len(approx)==4:
        docCnt=approx
        break
...
# show the original transform image and grayscale transform image
cv2.imshow("Transformed Image",paper)
cv2.imshow("Transformed Gray Image",wraped)
cv2.waitKey(0)
cv2.destroyAllWindows()

# apply Otsu's thresholding method to binarize the wraped piece of paper
...
# show the all questionCnts
outline=paper.copy()
cv2.drawContours(outline,questionCnts,-1,(0,255,0),2)
cv2.imshow("Outline",outline)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
for (q, i) in enumerate(np.arange(0, len(questionCnts), 5)):
    ...
    if bubbled is not None:
        ...
        # show the Marked Answer
        cv2.drawContours(paper, [cnts[k]], -1, color, 3)
        cv2.imshow("ANSWER",paper)
        cv2.waitKey(0)
        cv2.destroyAllWindows()    
